...
dyn_ranges_end = (100., 1000., 3000.) # mV
data_unf_res = []
...
for (dr_end, c) in product(dyn_ranges_end, pulse_charges)
    ...
    for t in pulse_times
        ...
        digi_wf = digitize_waveform(ps, pmt_config.sampling_freq, pmt_config.adc_freq, pmt_config.noise_amp, pmt_config.lp_filter, time_range=[-10, 50],  yrange=(0., dr_end),)
        unfolded = unfold_waveform(digi_wf, pmt_config.pulse_model_filt, pmt_config.unf_pulse_res, 0.1, :nnls)
        if length(unfolded) > 0
            if length(unfolded) > 1
                amax = sortperm(unfolded.charges)
                if amax[end] == length(unfolded.charges) || amax[end] == 1
                    amax = amax[end-1]
                else
                    amax = amax[end]
                end
...
for (grpkey, grp) in pairs(groupby(time_res, :dr_end))
    lines!(ax, grp[:, :charge], grp[:, :dt_iqr], label=string(grpkey[1]))
end
ylims!(ax, 0, 5)
Legend(fig[1, 2], ax, "Dynamic range (mV)")
fig
```
...
```{julia}
ps = PulseSeries([5.], [1.], pmt_config.pulse_model)
digi_wf = digitize_waveform(ps, pmt_config.sampling_freq, pmt_config.adc_freq, pmt_config.noise_amp, pmt_config.lp_filter, time_range=[-10, 50],  yrange=(0., 1000.),)
unfolded = unfold_waveform(digi_wf, pmt_config.pulse_model_filt, pmt_config.unf_pulse_res, 0.1, :nnls)
