...
pmt_config_low_noise = PMTConfig(
    st=ExponTruncNormalSPE(expon_rate=1.0, norm_sigma=0.3, norm_mu=1.0, trunc_low=0.0, peak_to_valley=3.1),
    pm=PDFPulseTemplate(
        dist=Truncated(Gumbel(0, gumbel_scale) + gumbel_loc, 0, 30),
        amplitude=7.0 # mV
...
for (dr_end, c) in product(dyn_ranges_end, pulse_charges)
    ...
    for t in pulse_times
        ps = PulseSeries([t], [c], pmt_config_low_noise.pulse_model)
        digi_wf = digitize_waveform(ps, pmt_config_low_noise.sampling_freq, pmt_config_low_noise.adc_freq, pmt_config_low_noise.noise_amp, pmt_config_low_noise.lp_filter, time_range=[-10, 50], yrange=(0.0, dr_end),)
        unfolded = unfold_waveform(digi_wf, pmt_config_low_noise.pulse_model_filt, pmt_config_low_noise.unf_pulse_res, 0.1, :nnls)
        if length(unfolded) > 0
            amax = sortperm(unfolded.charges)[end]
...
Legend(
    ...
    [string.(dyn_range_labels), [format("{:.2f}", pmt_config.noise_amp), format("{:.2f}", pmt_config_low_noise.noise_amp)]],
    ["Dynamic range (mV)", "Noise Amplitude (mV)"])
fig
```
